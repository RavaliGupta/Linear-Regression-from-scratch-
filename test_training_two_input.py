...
def run():
    ...
    z_training = np.array(points_training['Compactness'])
    ...
    print ("Running...")
    [thetha0, thetha1, thetha2] = gradient_descent_runner(x_training,y_training,z_training, initial_thetha0, initial_thetha1, initial_thetha2, learning_rate, num_iterations)
    max_x_training=np.amax(x_training)
    min_x_training=np.amin(x_training)
    mean_x_training=np.mean(x_training)
    max_z_training=np.amax(z_training)
    min_z_training=np.amin(z_training)
    mean_z_training=np.mean(z_training)
    feature_x_training= (x-mean_x_training)/(max_x_training-min_x_training)
    feature_z_training= (z-mean_z_training)/(max_z_training-min_z_training)
    plt.plot(x, thetha2*feature_z_training+thetha1*feature_x_training+thetha0, color='r')
    plt.show()
